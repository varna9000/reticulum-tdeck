...
**Cyrillic display support**: messages, names, and pages render Cyrillic (Bulgarian/Russian/Ukrainian/Belarusian) via a CP866-layout slot mapping shared by every display font (`lib/spleen_8x16.py` for the UI, `lib/spleen_6x12.py` and the `shell_*` grids for the rnsh screen, all generated by `tools/gen_shell_font.py`). Unsupported scripts (CJK, Greek, emoji) are stripped from display; keyboard input remains ASCII.
...
- **Shell terminal: 53 columns, and a modern font everywhere** — the rnsh screen was a 40×12 grid in the IBM VGA ROM font, so any command emitting an 80-column line wrapped and shredded its own layout. Neither display path in the st7789 C driver could fix that: both cost one SPI window-set per glyph (**~210 µs measured** on this board), and `text()` additionally requires a font width that is a multiple of 8 — which pins it to 40 columns on a 320px panel. The proportional `write()` path allows narrow glyphs but measured **565 ms** for an 80-column repaint. The shell now bypasses both, compositing each row into a `framebuf` and pushing it with a single `blit_buffer`: **53×16 in 83 ms**, against **135 ms** for the 40×12 screen it replaces — more text, drawn faster. Four grids are selectable live from the shell's control menu (53×16 / 53×19 / 64×24 / 80×32); each rewraps the local terminal and pushes a `WindowSizeMessage` so the remote pty follows. The system font is now **Spleen** (BSD-2) rather than the 1981 VGA ROM font, generated into the same CP437+CP866 slot layout by a new `tools/gen_shell_font.py`, with the few slots Spleen lacks (Є/є and the Bulgarian stressed Ѝ/ѝ) filled from the old font so nothing regressed. Two latent geometry bugs surfaced and were fixed: a font switch *during* the connect was dropped entirely, leaving the pty on the pre-switch size for the whole session, and a `WindowSizeMessage` refused by a busy Channel was never retried — both would have reproduced exactly the wrapping symptom this change set out to remove.
- **NET and SSH node lists are session-scoped** — both tabs used to be seeded from `Identity.known_destinations`, the persistent store of public keys. That store has to survive a reboot (it holds the keys needed to encrypt to a peer and to validate its announces), but a node recorded in it days ago says nothing about whether it is reachable *now*, so the tabs filled with stale hashes that could only be discovered dead by trying them. The lists now start empty at boot and hold exactly what has announced since — announces heard while running are added directly, including before a tab is first opened, so nothing is lost.
- **The SSH tab can finally reach `rnsh` listeners (submodule fix)** — links to any destination sitting **behind a shared instance** (`rnsd`) never established: the request was wrapped in a transport header and addressed "via" the instance, and the listener discarded it without logging a thing. Reference RNS inserts transport headers only when a destination is **more than one hop away**; urns used two different wrong criteria — the TCP interface wrapped whenever the destination was merely *present* in the path table, and the packet builder wrapped whenever `next_hop != destination`. Both misfire in exactly the same place, because a destination behind a shared instance *is* one hop away while its next hop is the instance's identity. The drop is silent by construction: the instance passes the packet to its local client with the header still attached (stripping is gated on `local_hops_delta`, off by default), and the client only accepts a link request whose transport ID is absent or matches its **own** — and shared-instance clients generate an ephemeral identity that never matches. Only link requests carry that check, which is why opportunistic LXMF kept working and disguised this as an rnsh problem for days. The same defect bites twice on one path: a transport node correctly strips a relayed request to HDR_1 for the final hop, then its own TCP egress puts the header straight back on — so **if you run a urns transport node with a TCP uplink, update it**, that is the node that needs the fix, not the leaf. Root-caused by running the real urns stack under CPython against the live listener and diffing its emitted bytes against reference RNS on the same socket (`102B flags=0x52` vs `86B flags=0x02`); verified end-to-end, link up in 180 ms, multi-hop unaffected. Five regression tests added — there were none for this decision, which is how a wrong rule shipped twice.
- **Tracks reference RNS 1.3.9 (submodule)** — the MCU-relevant hardening from upstream's 1.3.9 release is ported: resource advertisements are validated before anything is allocated (an unchecked size claim is an instant out-of-memory on an ESP32), resources verify their link is alive before every send, cancellations are signalled to the peer (`RESOURCE_ICL`/`RESOURCE_RCL`) instead of leaving it to re-advertise into a dead transfer for minutes of LoRa airtime, a link binds its remote identity only once, and the TCP interface drops malformed HDLC frames rather than feeding truncated packets to routing. Covered by a new host-side test suite. **If you run an `rnsh` listener anywhere on your mesh, update it** — 1.3.9 patches a critical hole where a command could run on a session that never completed identity authorisation. The T-Deck's rnsh support is client-side only and is not affected.
