/*
 * tjpgd_fast_xtensawin — built-in user C module (JPEG -> RGB565)
 *
 * Static conversion of tools/natmod/tjpgd_fast/main.c; same API and import
 * name as the .mpy it replaces (ui.py / tdeck_node.py import it directly).
 * Built into firmware: code runs from flash XIP.
 *
...
/* Our TJpgDec copy is compiled under prefixed names (see tjpgd_renamed.c)
 * to avoid colliding with the copy bundled inside st7789_mpy. */
...
static const mp_rom_map_elem_t tjpgd_fast_globals_table[] = {
    { MP_ROM_QSTR(MP_QSTR___name__), MP_ROM_QSTR(MP_QSTR_tjpgd_fast_xtensawin) },
    { MP_ROM_QSTR(MP_QSTR_decode), MP_ROM_PTR(&mod_decode_obj) },
    { MP_ROM_QSTR(MP_QSTR_info), MP_ROM_PTR(&mod_info_obj) },
};
static MP_DEFINE_CONST_DICT(tjpgd_fast_globals, tjpgd_fast_globals_table);

const mp_obj_module_t tjpgd_fast_user_cmodule = {
    .base = { &mp_type_module },
    .globals = (mp_obj_dict_t *)&tjpgd_fast_globals,
};

MP_REGISTER_MODULE(MP_QSTR_tjpgd_fast_xtensawin, tjpgd_fast_user_cmodule);
