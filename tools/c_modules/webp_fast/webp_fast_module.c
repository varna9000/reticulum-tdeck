/*
 * webp_fast_xtensawin — built-in user C module (WebP -> RGB565)
 *
 * Static conversion of tools/natmod/webp_fast/main.c; same API and import
 * name as the .mpy it replaces (ui.py imports it directly). Built into
 * firmware: code runs from flash XIP.
 *
...
static const mp_rom_map_elem_t webp_fast_globals_table[] = {
    { MP_ROM_QSTR(MP_QSTR___name__), MP_ROM_QSTR(MP_QSTR_webp_fast_xtensawin) },
    { MP_ROM_QSTR(MP_QSTR_decode), MP_ROM_PTR(&mod_decode_obj) },
    { MP_ROM_QSTR(MP_QSTR_info), MP_ROM_PTR(&mod_info_obj) },
};
static MP_DEFINE_CONST_DICT(webp_fast_globals, webp_fast_globals_table);

const mp_obj_module_t webp_fast_user_cmodule = {
    .base = { &mp_type_module },
    .globals = (mp_obj_dict_t *)&webp_fast_globals,
};

MP_REGISTER_MODULE(MP_QSTR_webp_fast_xtensawin, webp_fast_user_cmodule);
