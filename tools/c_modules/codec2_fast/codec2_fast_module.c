/*
 * codec2_fast_xtensawin — built-in user C module (Codec2 voice codec)
 *
 * Static conversion of tools/natmod/codec2_fast/main.c; same API and import
 * name as the .mpy it replaces (tdeck_node.py imports it directly). Built
 * into firmware: code runs from flash XIP, which also removes the natmod
 * IRAM-exhaustion hard-reset path that used to guard this import.
 *
...
static const mp_rom_map_elem_t codec2_fast_globals_table[] = {
    { MP_ROM_QSTR(MP_QSTR___name__), MP_ROM_QSTR(MP_QSTR_codec2_fast_xtensawin) },
    { MP_ROM_QSTR(MP_QSTR_create), MP_ROM_PTR(&mod_create_obj) },
    { MP_ROM_QSTR(MP_QSTR_decode_frame), MP_ROM_PTR(&mod_decode_frame_obj) },
    { MP_ROM_QSTR(MP_QSTR_encode_frame), MP_ROM_PTR(&mod_encode_frame_obj) },
    { MP_ROM_QSTR(MP_QSTR_destroy), MP_ROM_PTR(&mod_destroy_obj) },
    { MP_ROM_QSTR(MP_QSTR_decode), MP_ROM_PTR(&mod_decode_obj) },
    { MP_ROM_QSTR(MP_QSTR_encode), MP_ROM_PTR(&mod_encode_obj) },
    { MP_ROM_QSTR(MP_QSTR_info), MP_ROM_PTR(&mod_info_obj) },
    { MP_ROM_QSTR(MP_QSTR_debug_encode), MP_ROM_PTR(&mod_debug_encode_obj) },
};
static MP_DEFINE_CONST_DICT(codec2_fast_globals, codec2_fast_globals_table);

const mp_obj_module_t codec2_fast_user_cmodule = {
    .base = { &mp_type_module },
    .globals = (mp_obj_dict_t *)&codec2_fast_globals,
};

MP_REGISTER_MODULE(MP_QSTR_codec2_fast_xtensawin, codec2_fast_user_cmodule);
