...
struct CODEC2 *codec2_create(int mode)
{
    ...
    if (!CODEC2_MODE_ACTIVE(CODEC2_MODE_3200, mode) &&
    ...
    nlp_set_scratch(c2->nlp, c2->Fw_scratch);
...
/*---------------------------------------------------------------------------*\
  ...
  Decodes frames of 64 bits into 160 samples (20ms) of speech.
                        ...
                        const unsigned char *bits)
{
    ...
    int i, j;
    unsigned int nbit = 0;
    COMP *Aw = c2->Aw_scratch;  /* was stack-allocated, moved to struct */
    ...
    decode_lspds_scalar(&lsps[1][0], lsp_indexes, LPC_ORD);
...
float codec2_get_energy(struct CODEC2 *c2, const unsigned char *bits)
{
    ...
    if (CODEC2_MODE_ACTIVE(CODEC2_MODE_3200, c2->mode)) {
        nbit = 1 + 1 + WO_BITS;
        index = unpack(bits, &nbit, E_BITS);
        e = decode_energy(index, E_BITS);
    }

    if (CODEC2_MODE_ACTIVE(CODEC2_MODE_2400, c2->mode)) {
        nbit = 1 + 1;
        index = unpack(bits, &nbit, WO_E_BITS);
        decode_WoE(&c2->c2const, &model, &e, xq_dec, index);
    }
...
void analyse_one_frame(struct CODEC2 *c2, MODEL *model, short speech[])
{
    COMP *Sw = c2->Sw_scratch;  /* was stack-allocated, moved to struct */
    float pitch;
    int i;
