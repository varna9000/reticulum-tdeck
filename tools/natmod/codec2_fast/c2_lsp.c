...
{
    /* Clenshaw algorithm in double precision — prevents sign-change
       detection failures in lpc_to_lsp on single-precision platforms. */
    int m = order / 2;
    double b1 = 0.0, b2 = 0.0, tmp;
    int k;

    for (k = m; k >= 1; k--) {
        tmp = b1;
        b1 = 2.0 * x * b1 - b2 + coef[m - k];
        b2 = tmp;
    }
    return coef[m] + x * b1 - b2;
}
...
{
    /* Use double precision throughout to prevent sign-change detection
       failures on ESP32/Xtensa single-precision float platforms. */
    ...
    /* Use double for P/Q polynomials — single-precision causes
       root-finding failures on ESP32/Xtensa. */
    double Q[LPC_ORD + 1];
    double P[LPC_ORD + 1];
    ...
    *qx++ = 1.0;
    for (i = 1; i <= m; i++) {
        *px++ = (double)a[i] + (double)a[order + 1 - i] - *p++;
        *qx++ = (double)a[i] - (double)a[order + 1 - i] + *q++;
    }
    px = P;
    qx = Q;
    for (i = 0; i < m; i++) {
    ...
    for (j = 0; j < order; j++) {
        ...
        flag = 1;
        while (flag && (xr >= -1.0)) {
            /* Adaptive step: narrow near edges where roots cluster
               (from Speex — roots dense near xl = ±1.0) */
            {
                double dd = ddelta * (1.0 - 0.9 * xl * xl);
                if (psuml < 0.2 && psuml > -0.2)
                    dd *= 0.5;
                xr = xl - dd;
            }
