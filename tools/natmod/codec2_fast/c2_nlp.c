...
typedef struct {
    ...
    COMP *Fw_scratch;        /* workspace (from CODEC2 struct, avoids stack) */
    FILE *f;
} NLP;
...
void nlp_set_scratch(void *nlp_state, COMP *Fw_scratch)
{
    NLP *nlp = (NLP *)nlp_state;
    nlp->Fw_scratch = Fw_scratch;
}
...
)
{
    ...
    assert(nlp_state != NULL);
    nlp = (NLP *)nlp_state;
    Fw = nlp->Fw_scratch;  /* was stack-allocated, moved to struct */
    m = nlp->m;
