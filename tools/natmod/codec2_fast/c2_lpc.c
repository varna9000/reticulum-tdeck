                  ...
                  )
{
    ...
    /* Double-precision accumulation prevents precision loss when
       summing many small products (normalized window ~0.004). */
    for (j = 0; j < order + 1; j++) {
        double acc = 0.0;
        for (i = 0; i < Nsam - j; i++) {
            acc += (double)Sn[i] * (double)Sn[i + j];
        }
        Rn[j] = (float)acc;
    }
}

void autocorrelate_d(float Sn[], double Rn[], int Nsam, int order)
{
    int i, j;
    for (j = 0; j < order + 1; j++) {
        double acc = 0.0;
        ...
        Rn[j] = acc;  /* keep full double precision */
    }
}
...
{
    /* Entire recursion in double precision to prevent catastrophic
       cancellation on single-precision float platforms (ESP32 Xtensa).
       Software double on ESP32 is ~5x slower but only runs once per
       20ms frame — negligible vs FFT cost. */
    double sum, e, k;
    int i, j;
