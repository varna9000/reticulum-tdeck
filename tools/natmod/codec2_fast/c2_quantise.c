...
#define LSP_DELTA1 0.02 /* grid spacing for LSP root searches (coarse — adaptive step refines) */

/* Debug counters for LSP root finding */
int _lsp_calls = 0, _lsp_fails = 0, _lsp_last_roots = 0;
                        ...
                        int m_pitch, int order)
{
    ...
    /* Use double for autocorrelation to preserve precision in R[j]/R[0]
       ratios — critical for accurate LPC and LSP root-finding on ESP32. */
    double R[LPC_ORD + 1];
    assert(order == LPC_ORD);
    ...
    {
        static float prev_lsps[LPC_ORD];
        ...
        if (roots != order) {
            ...
            if (!prev_valid) {
                for (i = 0; i < order; i++)
                    prev_lsps[i] = (PI / order) * (float)i;
                prev_valid = 1;
            }
            /* Keep found roots, fill missing from previous frame */
            for (i = roots; i < order; i++) {
                lsp[i] = prev_lsps[i];
            }
